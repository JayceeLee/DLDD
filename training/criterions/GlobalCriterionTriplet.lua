...
function GlobalCriterionTriplet:updateGradInput(input)
   ...
   self.gradInput[1] = (torch.cmul(torch.add(self.d_plus, -self.mean_plus):expandAs(a),a-p) + torch.cmul(torch.add(self.d_minus, -self.mean_minus):expandAs(a),a-n) - torch.mul((p-n),0.5*self.lambda* (((self.mean_minus - self.mean_plus - self.alpha) < 0) and 1 or 0)))/N
   
 
   self.gradInput[2] = (torch.cmul(torch.add(self.d_plus, -self.mean_plus):expandAs(a),p-a) - torch.mul((a-p),0.5*self.lambda* (((self.mean_minus - self.mean_plus - self.alpha) < 0) and 1 or 0)))/N

   self.gradInput[3] = (torch.cmul(torch.add(self.d_minus, -self.mean_minus):expandAs(a),n-a) - torch.mul((n-a),0.5*self.lambda* (((self.mean_minus - self.mean_plus - self.alpha) < 0) and 1 or 0)))/N
