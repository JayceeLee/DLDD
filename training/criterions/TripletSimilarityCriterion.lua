...
function TripletSimilarityCriterion:updateGradInput(input)
   ...
   local N = a:size(1)
   self.gradInput = {}
   self.gradInput[1] = torch.cmul(n - p, self.Li:gt(0):repeatTensor(1, a:size(2)):type(a:type()) * 1/N)
   self.gradInput[2] = torch.cmul(-a, self.Li:gt(0):repeatTensor(1, a:size(2)):type(a:type()) * 1/N)
   self.gradInput[3] = torch.cmul(a, self.Li:gt(0):repeatTensor(1, a:size(2)):type(a:type()) * 1/N)
