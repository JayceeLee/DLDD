...
local function findImages(dir, classToIdx)
   ...
   local imagePaths = {}
   local imageClasses = {}
   local classListSample  = {}
   for idx=1,tablelength(classToIdx) do
    classListSample[idx] = {}
   end
   ...
   while true do
      ...
      table.insert(classListSample[classId], idxImage)
   ...
   return imagePath, imageClass, classListSample
end
...
function M.exec(opt, cacheFile)
   ...
   local valImagePath, valImageClass, valImageClassSample = findImages(valDir, classToIdx)

   print(" | finding all training images")
   local trainImagePath, trainImageClass, trainImageClassSample = findImages(trainDir, classToIdx)
   ...
   local info = {
      ...
      train = {
         ...
         labels = trainImageClass,
	   classSample = trainImageClassSample,
      },
      val = {
         imagePath = valImagePath,
         labels = valImageClass,
	 classSample = valImageClassSample,
      },
   }
